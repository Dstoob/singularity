    ...
    for sounds_path in sounds_paths:
        ...
        for entry in os.walk(sounds_path):
            ...
            for file_name in files:
                ...
                if len(file_name) < 6 or file_name[-3:] != "wav":
                    continue
                ...
                sounds[sound_class].append({"filename": real_file, "sound": sound})
...
def play_sound(sound_class, sound_volume="gui"):
    ...
    if sound_class not in sounds:
        sys.stderr.write(
            "WARNING: Requesting a sound of unavailable class: %s\n" % sound_class
        )
        return
